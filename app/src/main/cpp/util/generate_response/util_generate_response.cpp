...
std::string generate_response(const std::string& prompt, int maxTokens, llama_context* ctx) {
    ...
    tokens.resize(prompt.size() + 32); // Reserve extra space (text might become more tokens than characters)
    ...
    for (int i = 0; i < n_input; i++) {
        ...
        batch.logits[i] = (i == n_input - 1) ? 1 : 0;       // Only calculate predictions for the last token
    }
