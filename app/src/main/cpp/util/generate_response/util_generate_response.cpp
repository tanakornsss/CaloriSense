...
std::string generate_response(const std::string &prompt, int maxTokens, llama_context *ctx) {
    ...
    llama_sampler_chain_add(sampler,
                            llama_sampler_init_top_p(0.9f, 1)); // Consider 90% probability mass
    llama_sampler_chain_add(sampler,
                            llama_sampler_init_temp(0.8f));     // Add controlled randomness
    llama_sampler_chain_add(sampler, llama_sampler_init_dist(
            12345));    // Final selection (with seed for consistency)
